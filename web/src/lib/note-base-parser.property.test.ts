 ...
 * Includes all required columns so the parser doesn't reject the sheet.
 */
...
/**
 * Base required columns that must exist in every test row for the parser to accept the sheet.
 */
const REQUIRED_BASE_ROW: Record<string, unknown> = {
  '内容方向': '种草',
  '笔记类型': '图文',
  '资源含税成本价': 5000,
  '资源含税售价': 6000,
