...
- [x] 1. 数据库Schema变更与迁移
  ...
  - [x] 1.2 扩展 User 表角色体系
    - 修改 `prisma/schema.prisma` 的 User model，将 `role` 字段值域从 `"admin" | "user"` 扩展为 `"admin" | "组长" | "AD" | "AM" | "投手" | "执行"`
    - 保持现有字段不变，仅扩展 role 的业务含义
    - _Requirements: 3.5.1, 3.5.5_

  - [x] 1.3 新增 ProjectTreeNode 表
...
- [x] 2. Checkpoint - 确认数据库迁移成功
  - Ensure all tests pass, ask the user if questions arise.

- [x] 3. 后端API - 树结构与项目底表导入
  - [x] 3.1 实现树结构查询 API
...
- [x] 17. 前端页面 - 新建复盘
  - [x] 17.1 创建新建复盘页面
    ...
    - 项目信息区域：项目选择器（从已创建项目列表中选择，支持搜索），选择后自动回填品类/品牌/业务线（只读展示）
  ...
  - [x] 17.2 Property test: 报告模块开关独立性
...
- [x] 24. 用户批量导入功能
  - [x] 24.1 实现用户批量导入 API
    - 创建 `web/src/app/api/admin/import/users/route.ts`
    - 接受 .xlsx 文件，解析用户名、显示名、角色（组长/AD/AM/投手/执行）字段
    - 为每个用户生成初始密码，创建User记录（mustChangePassword=true）
    - 返回 `{ imported, errors }`
    - _Requirements: 3.5.5, 3.5.6_

  - [x] 24.2 在账户管理页面集成用户批量导入
    - 修改 `web/src/app/admin/users/page.tsx`
    - 添加Excel上传区域，调用 `/api/admin/import/users`
    - 显示导入结果（成功数量、失败原因）
    - _Requirements: 3.5.6_

- [x] 25. Final checkpoint - 全部功能验证
  - Ensure all tests pass, ask the user if questions arise.
