...
def _resolve_nextjs_base_url() -> str:
    explicit_candidates = (
        os.getenv("NEXTJS_INTERNAL_URL", "").strip(),
        os.getenv("NEXT_PUBLIC_URL", "").strip(),
    )
...
def _build_template_fallback_headers() -> dict[str, str] | None:
...
async def _fetch_template_fallback_payload(
    ...
    nextjs_base = _resolve_nextjs_base_url()
    # Strip trailing slash and ensure we hit the Next.js /api/template route
    nextjs_origin = nextjs_base.rstrip("/")
    # In Docker, nginx on port 80 proxies both Next.js and FastAPI.
    # In local dev, Next.js runs on its own port (e.g. 5000).
    fallback_url = f"{nextjs_origin}/api/template?group={layout_name}"
    LOGGER.info(
        "[template_layout] trying HTTP fallback template=%r url=%s",
        layout_name,
        fallback_url,
    )
    headers = _build_template_fallback_headers()
    try:
