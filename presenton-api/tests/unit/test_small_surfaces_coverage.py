...
def test_get_app_data_directory_env_defaults_to_local_app_data(monkeypatch, tmp_path):
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("APP_DATA_DIRECTORY", raising=False)

    assert get_app_data_directory_env() == os.path.abspath(
        os.path.join(str(tmp_path), "app_data")
    )


def test_get_user_config_path_env_defaults_under_app_data(monkeypatch, tmp_path):
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("APP_DATA_DIRECTORY", raising=False)
    monkeypatch.delenv("USER_CONFIG_PATH", raising=False)

    assert get_user_config_path_env() == os.path.abspath(
        os.path.join(str(tmp_path), "app_data", "userConfig.json")
    )
...
def test_export_prefers_internal_nextjs_url_for_local_runtime():
    async def runner():
        fake_result = MagicMock(path="/exports/deck.pdf")
        dummy = uuid.uuid4()
        mock_pdf = AsyncMock(return_value=fake_result)
        ...
        with patch.dict(
            os.environ,
            {
                "NEXTJS_INTERNAL_URL": "http://127.0.0.1:5000",
                "NEXT_PUBLIC_URL": "https://public.example",
        ...
        pdf_call = mock_pdf.await_args.kwargs
        assert pdf_call["url"].startswith("http://127.0.0.1:5000/pdf-maker?")
        assert pdf_call["fastapi_url"] == "http://127.0.0.1:8000"
...
def test_resolve_nextjs_base_url_prefers_dev_default(monkeypatch):
    monkeypatch.delenv("NEXTJS_INTERNAL_URL", raising=False)
    monkeypatch.delenv("NEXT_PUBLIC_URL", raising=False)
    monkeypatch.setenv("NODE_ENV", "development")

    assert tpl_layout_fetcher._resolve_nextjs_base_url() == "http://127.0.0.1:5000"
