...
async def generate_presentation_handler(
    ...
    try:
        ...
        if not isinstance(e, HTTPException):
            logger.exception(
                "Presentation generation crashed presentation_id=%s",
                presentation_id,
            )
            e = HTTPException(status_code=500, detail="Presentation generation failed")
        else:
            logger.error(
                "Presentation generation failed presentation_id=%s status=%s detail=%s",
                presentation_id,
                e.status_code,
                e.detail,
            )
...
@PRESENTATION_ROUTER.post("/export/pptx", response_model=PresentationAndPath)
async def export_presentation_as_pptx(
    ...
    title = data.title or presentation.title or str(uuid.uuid4())
